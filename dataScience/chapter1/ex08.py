...
- groupby는 "반 별로 나눈다 -> 나눈 그룹마다 계산한다 -> 다시 합친다(split-apply-combine)"
...
# 3. agg() : 반별 점수에 대한 (평균, 최대값, 최소값, 인원수)를 한 번에
summary = df.groupby("반")["점수"].agg(["mean","max","min","count"])
print("\n=== 반별 통계 요약(평균, 최대값, 최소값, 인원수) ===")
print(summary)

# 4. 과목별로 어떤 반이 가장 잘 했는지 확인(표 형태로 반환)
