...
load_dotenv() # .env 호출
SAVE_ROOT = os.getenv("PY_SCRAP_SAVE")

# 유튜브 영상 페이지에 Playwright로 접속해서 무한 스크롤로 모든 댓글을 로딩시킨 뒤, 
# 각 댓글의 작성자·내용·좋아요 수·프로필 이미지를 추출해서 엑셀 파일로 저장

workbook = xlsxwriter.Workbook(SAVE_ROOT+'you_crawl_result.xlsx')
    ...
    for dom in comment :
        try :
            ...
            if img_src and img_src.startswith('http') :
                try :
                    request = req.Request(img_src,headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'})
